...
public interface TimelineReader {
     ...
     * @param limit            A limit on the number of entities to return. If null, defaults to
     *                         {@link #DEFAULT_LIMIT}.
     * @param windowStart      The earliest start timestamp to retrieve (exclusive). If null,
     *                         defaults to retrieving all entities until the limit is reached.
     * @param windowEnd        The latest start timestamp to retrieve (inclusive). If null,
     *                         defaults to {@link Long#MAX_VALUE}
     * @param fromId           If fromId is not null, retrieve entities earlier than and
     ...
     * @param fromTs           If fromTs is not null, ignore entities that were inserted into the
     ...
     * @param primaryFilter    Retrieves only entities that have the specified primary filter. If
     *                         null, retrieves all entities. This is an indexed retrieval, and no
     *                         entities that do not match the filter are scanned.
     * @param secondaryFilters Retrieves only entities that have exact matches for all the
     ...
     * @param fieldsToRetrieve Specifies which fields of the entity object to retrieve (see
     ...
     *
     * @param entityId         The entity whose information will be retrieved.
     ...
     * @param limit       A limit on the number of events to return for each entity. If
     *                    null, defaults to {@link #DEFAULT_LIMIT} events per entity.
     * @param windowStart If not null, retrieves only events later than the given time
     *                    (exclusive)
     * @param windowEnd   If not null, retrieves only events earlier than the given time
     *                    (inclusive)
     * @param eventTypes  Restricts the events returned to the given types. If null, events
